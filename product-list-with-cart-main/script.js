document.addEventListener('DOMContentLoaded', () => {
    ...
    const initializeCart = () => {
        document.querySelectorAll('.products-card').forEach(card => {
            ...
            const productPrice = parseFloat(card.querySelector('.price').textContent.replace('R', ''));
            ...
            cartItem.innerHTML = `
                <div>
                    ...
                    <div class="pdcs-infos">
                        ...
                        <p class="single-price">@ R${product.price.toFixed(2)}</p>
                        <p class="total-price">R${(product.quantity * product.price).toFixed(2)}</p>
                    </div>
                </div>
    ...
    confirmOrderButton.addEventListener('click', () => {
            ...
            if (productInfo) {  // Check if productInfo is defined
                summaryHtml += `
                    <div class="summary-item">
                        ...
                        <p class="final-price">R${(product.quantity * product.price).toFixed(2)}</p>
                    </div>
                `;
                totalPrice += product.quantity * product.price;
            }
        ...
        summaryHtml += `<p class ="final-total">Order Total <strong>R${totalPrice.toFixed(2)}</strong></p>`;
